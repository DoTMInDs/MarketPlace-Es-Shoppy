...
def product_detail(request, product_id):
    product = get_object_or_404(Product.objects.prefetch_related('productspecification_set__specification'), id=product_id)
    cart = None
...
def update_quantity(request, item_id, action):
    ...
    try:
        order_product = OrderProduct.objects.get(id=item_id, order__buyer=request.user, order__status='cart')
        if action == 'increment':
            order_product.quantity += 1
        elif action == 'decrement' and order_product.quantity > 1:
            order_product.quantity -= 1
        ...
        return JsonResponse({
            'success': True,
            'quantity': order_product.quantity,
            'total_price': order_product.order.total_price
        })
